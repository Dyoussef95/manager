<x-app-layout>
    ...
    <a class="btn btn-warning py-2" href="{{ route('ubicaciones.index')}}"> Volver atras </a>
    <form action="{{ route('ubicaciones.update', $ubicacion->id)  }}" method="POST" role="form" id="form">
            <legend>Actualizar ubicación: {{ isset($ubicacion) ? $ubicacion->name : '' }}</legend>
            @method('PATCH')
            @csrf
            ...
            <input type="text" name="name" value="{{ isset($ubicacion) ? $ubicacion->name : '' }}"
            class="form-control" id="" placeholder="Ingrese el nombre">
            </div>

            <br>
            <a type="button" class="btn btn-danger" href="{{ route('ubicaciones.index') }}">Cancelar</a>
