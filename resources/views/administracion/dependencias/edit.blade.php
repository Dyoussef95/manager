    ...
    <li><a href="{{route('users')}}" class="nav-link px-2 link-body-emphasis">Colaboradores</a></li>
       <li class="nav-item dropdown">
        ...
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Dependencias / Departamentos') }}
        </h2>
    </x-slot>

    <a class="btn btn-warning py-2" href="{{ route('dependencias.index')}}"> Volver atras </a>
    <form action="{{ route('dependencias.update', $dependencia->id)  }}" method="POST" role="form" id="form">
            <legend>Actualizar Dependencia / Departamento: {{ isset($dependencia) ? $dependencia->name : '' }}</legend>
            @method('PATCH')
            @csrf
            ...
            <input type="text" name="name" value="{{ isset($dependencia) ? $dependencia->name : '' }}"
            class="form-control" id="" placeholder="Ingrese el nombre">
            </div>

            <br>
            <a type="button" class="btn btn-danger" href="{{ route('dependencias.index') }}">Cancelar</a>
