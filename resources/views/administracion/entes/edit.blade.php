<x-app-layout>
    ...
    <a class="btn btn-warning py-2" href="{{ route('entes.index')}}"> Volver atras </a>
    <form action="{{ route('entes.update', $ente->id)  }}" method="POST" role="form" id="form">
            <legend>Actualizar ente: {{ isset($ente) ? $ente->name : '' }}</legend>
            @method('PATCH')
            @csrf
            ...
            <input type="text" name="name" value="{{ isset($ente) ? $ente->name : '' }}"
            class="form-control" id="" placeholder="Ingrese el nombre">
            </div>

            <br>
            <a type="button" class="btn btn-danger" href="{{ route('entes.index') }}">Cancelar</a>
